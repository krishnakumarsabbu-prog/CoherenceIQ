...
import { Code as Code2, FileJson, SquareFunction as FunctionSquare, Play, CircleCheck as CheckCircle2, TriangleAlert as AlertTriangle, Circle as XCircle, GitBranch, Clock, Check, X, ArrowUpRight, History, FlaskConical, ShieldCheck } from "lucide-react";
...
function nodeToExpression(n: TreeNode): string {
  if (isGroup(n)) {
    const body = n.children.map(nodeToExpression).join(` ${n.op} `);
    const wrapped = n.children.length > 1 ? `(${body})` : body;
  ...
  return `$${n.variable} ${sym} ${/^[a-zA-Z0-9_.\-]+$/.test(n.value) ? n.value : `"${n.value}"`}`;
}

function nodeToJson(n: TreeNode, indent = 2): string {
  const pad = " ".repeat(indent);
  const pad2 = " ".repeat(indent + 2);
  if (isGroup(n)) {
    const inner = n.children.map((c) => nodeToJson(c, indent + 2)).join(",\n");
    return `${pad}{\n${pad2}"type": "${n.negated ? "not_" : ""}${n.op.toLowerCase()}",\n${pad2}"children": [\n${inner}\n${pad2}]\n${pad}}`;
  }
  return `${pad}{\n${pad2}"variable": "${n.variable}",\n${pad2}"op": "${n.op}",\n${pad2}"value": ${JSON.stringify(n.value)},\n${pad2}"plugin": "${n.plugin ?? ""}"\n${pad}}`;
}

function ruleToJson(rule: RiskRule): string {
...
function ruleToExpression(rule: RiskRule): string {
...
function validateRule(rule: RiskRule): ValidationItem[] {
  const items: ValidationItem[] = [];
  if (!rule.name.trim()) items.push({ level: "error", message: "Rule name is empty." });
  let conds = 0, emptyGroups = 0;
  const walk = (n: TreeNode) => {
    if (isGroup(n)) { if (n.children.length === 0) emptyGroups++; n.children.forEach(walk); }
  ...
  if (rule.actions.length === 0) items.push({ level: "error", message: "Rule has no actions." });
  if (rule.actions.some((a) => (a.kind === "Risk Increase" || a.kind === "Risk Reduction") && (!a.amount || a.amount <= 0))) {
...
function runSimulation(rule: RiskRule): { label: string; value: string; tone: string }[] {
  const s = rule.simulation;
  const matchRate = ((s.flagged / s.samples) * 100).toFixed(2);
  const precision = ((s.flagged / (s.flagged + s.failed)) * 100).toFixed(1);
  return [
    { label: "Sessions sampled", value: s.samples.toLocaleString(), tone: "text-foreground" },
    { label: "Rule matched", value: s.flagged.toLocaleString(), tone: "text-primary" },
    { label: "Match rate", value: `${matchRate}%`, tone: "text-primary" },
    { label: "Correctly blocked", value: s.flagged.toLocaleString(), tone: "text-success" },
    { label: "False positives", value: s.failed.toLocaleString(), tone: "text-warning" },
    { label: "Precision", value: `${precision}%`, tone: "text-success" },
  ];
}

interface Props { rule: RiskRule; onPublish: () => void; onApprove: (id: string) => void; onReject: (id: string) => void; }

export function RuleEditor({ rule, onPublish, onApprove, onReject }: Props) {
